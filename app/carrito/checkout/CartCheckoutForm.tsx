...
import { submitCartCheckoutAction } from '../../lib/actions';
...
export default function CartCheckoutForm({ items, user }: Props) {
    ...
    const moneda = items[0]?.curso?.moneda === 'USD' ? 'USD' : '$';
    ...
    if (step === 'ok') {
        return (
            <div className="bg-surface-site rounded-3xl shadow-xl border border-border-site p-10 text-center">
                <div className="text-5xl mb-4">✅</div>
                <h2 className="text-2xl font-bold mb-2 display-font">¡Solicitud enviada!</h2>
                <p className="text-text-site/60 mb-6">
                    Recibimos tu comprobante. Un administrador revisará tu pago y habilitará el acceso a los cursos en las próximas horas.
                </p>
                <button
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8">
            {/* ── RESUMEN DEL CARRITO ─────────────────────────── */}
            <section className="bg-surface-site rounded-3xl shadow-xl border border-border-site p-8">
                <h2 className="text-lg font-bold uppercase tracking-widest mb-6 border-b border-border-site pb-4">
                    Resumen de tu compra
                </h2>
                <div className="space-y-3">
                    {items.map(item => (
                        <div key={item.id} className="flex items-center justify-between text-sm">
                            <span className="text-text-site">{item.curso.titulo}</span>
                            <span className="font-mono font-bold text-text-site/70">
                                {item.curso.moneda === 'USD' ? 'US$' : '$'}{new Intl.NumberFormat('es-AR').format(item.curso.precio)}
                            </span>
                        </div>
                    ))}
                </div>
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-border-site">
                    <span className="font-bold uppercase tracking-widest text-sm">Total</span>
                    <span className="text-2xl font-black text-accent">{moneda} {new Intl.NumberFormat('es-AR').format(total)}</span>
                </div>
            </section>
            ...
            <section className="bg-surface-site rounded-3xl shadow-xl border border-border-site p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <select
                            name="pais"
                            className="bg-gray-50 dark:bg-black/20 border border-border-site rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
                        >
                            ...
                            <button
                                ...
                                className={`px-4 py-2 rounded-full border text-sm font-semibold transition-all ${
                                    redes.includes(red)
                                        ...
                                        : 'border-border-site text-gray-600 dark:text-gray-300 hover:border-primary'
                                }`}
                            >
                                {red}
                            </button>
                ...
                <div className="mt-6">
                    ...
                    <select
                        name="como_enteraste"
                        className="w-full bg-gray-50 dark:bg-black/20 border border-border-site rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
                    >
                ...
                <div className="mt-6">
                    ...
                    <textarea
                        ...
                        className="w-full bg-gray-50 dark:bg-black/20 border border-border-site rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all resize-none"
                    />
                </div>
            </section>
            ...
            <section className="bg-surface-site rounded-3xl shadow-xl border border-border-site p-8">
                ...
                {/* Datos bancarios / CVU */}
                {medioPago && (
                    <div className="bg-gray-50 dark:bg-black/20 rounded-2xl border border-border-site p-6 space-y-4">
                        ...
                        {medioPago === 'mercadopago' ? (
                            <>
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 bg-surface-site border border-border-site rounded-xl px-4 py-3">
                ...
                {medioPago && (
                    <div className="mt-6">
                        ...
                        <div
                            className="border-2 border-dashed border-border-site rounded-2xl p-6 text-center cursor-pointer hover:border-primary transition-colors"
        ...
        <button
            ...
            className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all font-semibold text-left ${
                selected
                    ...
                    : 'border-border-site hover:border-primary/50'
            }`}
        >
